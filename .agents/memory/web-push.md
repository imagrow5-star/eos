...
description: VAPID keys in DB, the 2/day cap race lesson, atomic morning dedup, iOS constraints
---
- VAPID keypair lives in DB `push_config` (id=1, ON CONFLICT DO NOTHING), per-environment — NOT secrets. Test seams exist: `_setWebPushForTests(mock|null)` + `_clearVapidCacheForTests()`.
