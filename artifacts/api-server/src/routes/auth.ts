...
router.delete("/auth/account", async (req, res): Promise<void> => {
  ...
  try {
    ...
    await client.query(`DELETE FROM stories           WHERE user_id = $1`, [userId]); // the person's own words — deleted means deleted
    await client.query(`DELETE FROM story_drops       WHERE user_id = $1`, [userId]); // dropped drafts quote them too
