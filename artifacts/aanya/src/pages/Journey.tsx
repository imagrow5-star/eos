...
export default function Journey() {
  ...
  const habitCorrelation = (() => {
    ...
    return { diff, habitNames: activeHabitNames.slice(0, 2) };
  })();

  // Best habit streak for the stats row
  ...
  return (
    <div className="h-full overflow-y-auto px-6 py-10 pb-20 space-y-12">
        ...
        {habitCorrelation && (
          <div className="flex items-start gap-2 px-4 py-3 rounded-xl bg-emerald-500/6 border border-emerald-500/15">
            ...
            <p className="text-[12px] text-muted-foreground/70 leading-relaxed">
              Your mood score is up <span className="text-emerald-400/80 font-medium">+{habitCorrelation.diff} points</span> while you've been keeping up with{" "}
              <span className="text-secondary/70">{habitCorrelation.habitNames.join(" and ")}</span>.
              What you do shows up in how you feel.
            </p>
          </div>
        )}
      </section>
      ...
      {/* ── Daily Routines / Habits (prominent) ────────────────────────────── */}
      <HabitsSection />
